...
func DateToDateIndoFormat(value time.Time) (time.Time, error) {
	var dateNewRow time.Time

	dateString := value.Format("2006-01-02")

	date, err := time.Parse("2006-01-02", dateString)
	if err != nil {
		return dateNewRow, err
	}
	dateNew := date.Format("02-01-2006")
	tanggals, _ := time.Parse("02-01-2006", dateNew)
	if err != nil {
		return dateNewRow, err
	}
	return tanggals, nil
...
func DateTmeToDateTimeIndoFormat(value time.Time) (time.Time, error) {
	var dateNewRow time.Time
...
func TempatTanggalLahirFormatIndonesia(tempat string, waktu time.Time) (string, error) {
	...
	// data := fmt.Sprintln("%s, $s", tempat, tanggal)
	return data, nil
}
